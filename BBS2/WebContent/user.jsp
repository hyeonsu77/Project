...
<%
UserDAO dbBean = UserDAO.getInstance();
List<User> memList = dbBean.getMembers();
%>
...
<!--  
  ...
  <source src="홈페이지음악.ogg" type="audio/ogg" />
  <source src="image/홈페이지음악.mp3" type="audio/mpeg" />
  </audio>
  -->
		<%
			//로그인 시 session 값 부여
			...
			<% 
			if (userID == null || !userID.equals("관리자")) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('관리자 만 접근 가능합니다.')");
	...
	<div id="wrapper">
			<div id="header_wrap">
					<div class="header">
							<h1><a href="/"><img src="http://localhost:18090/BBS2/image/LOGO2.jpg" alt="" class="logo"></a></h1>
							<p class="mMeun_btn">
								<span></span>
							</p>
							<div id="m_bg"></div>
				...
				<table class="table table-striped"
					...
					<thead>
						<tr>
							...
							<th style="background-color: #eeeeee; text-align: center;">회원관리</th>
						</tr>
					</thead>
					<tbody>
						<%
							for(int i=0; i<memList.size(); i++){
								User member = memList.get(i);
						%>
						<tr>
							<td><%=member.getUserID()%></td>
							<td><%=member.getUserName()%></td>
							<td><%=member.getUserPassword()%></td>
							<td><%=member.getUserGender()%>
							<td><%=member.getUserEmail()%></td>
							<td>
						<!--  	<form action ="AdminUserDelete.jsp">-->
							 <input type=button value=수정 onClick="location.href='AdminUserDelete.jsp?userId=<%=member.getUserID()%>'">
							 <input type=button value=삭제 onClick="location.href='AdminUserDelete.jsp?userId=<%=member.getUserID()%>'">
		                    </td>
					  </tr>	
